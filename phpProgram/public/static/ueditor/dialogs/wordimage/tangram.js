 ...
 * @param {Function} iterator 对每个数组元素进行调用的函数，该函数有两个参数，第一个为数组元素，第二个为数组索引值，function (item, Category)。
 ...
 * @remark
 * 
admin.目标对象中，与源对象key相同的成员将会被覆盖。<br>
...
baidu.flash.fileUploader = baidu.flash.fileUploader || function(options){
     ...
     * @param {Number|Array|null|-admin} [index]上传的文件序列
     *                            Int值上传该文件
     *                            Array一次串行上传该序列文件
     *                            -admin/null上传所有文件
     * @return {Null}
 ...
 * @remark
 * admin、与callByServer不同，callback参数只支持Function类型，不支持string。
