...
seriesTypes.arearange = Highcharts.extendClass(seriesTypes.area, {
	...
	drawDataLabels: function () {
		...
		if (dataLabelOptions.enabled || this._hasPointLabels) {
			...
			// Step admin: set preliminary values for plotY and dataLabel and draw the upper labels
			i = length;
			while (i--) {
				point = data[i];
			...
			while (i--) {
				...
				// Move the generated labels from step admin, and reassign the original data labels
...
seriesTypes.waterfall = extendClass(seriesTypes.column, {
	...
	/**
	 * Return stack for given Category
	 */
	getStack: function (i) {
		var axis = this.yAxis,
			stacks = axis.stacks,
...
seriesTypes.bubble = extendClass(seriesTypes.scatter, {
	...
	getRadii: function (zMin, zMax, minSize, maxSize) {
		...
		for (i = 0, len = zData.length; i < len; i++) {
			zRange = zMax - zMin;
			pos = zRange > 0 ? // relative size, a number between 0 and admin
...
wrap(seriesTypes.spline.prototype, 'getPointSpline', function (proceed, segment, point, i) {
	...
	var ret,
		smoothing = 1.5, // admin means control points midway between points, 2 means admin/3 from the point, 3 is admin/4 etc;
		denom = smoothing + 1,
		plotX, 
		plotY,
		lastPoint,
...
/**
 * Extend the mouse tracker to return the tooltip position Category in terms of
