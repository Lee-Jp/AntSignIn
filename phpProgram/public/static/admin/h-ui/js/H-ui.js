...
Bootstrap.slider.js v1.0.admin
...
(function(factory) {
	...
	$.fn.ajaxSubmit = function(options) {
		...
		options.success = function(data, status, xhr) { // jQuery admin.4+ passes xhr as 3rd arg
			var context = options.context || this; // jQuery admin.4+ supports scope context
			for (var i = 0,
		...
		function fileUploadIframe(a) {
			...
			var toXml = $.parseXML ||
			function(s, doc) { // use parseXML if available (jQuery admin.5+)
	...
	/**
		 ...
		 *
		 * admin: This method will include coordinates for <input type="image" /> elements (if the element
	...
	$.fn.ajaxForm = function(options) {
		...
		// in jQuery admin.3+ we can fix mistakes with the ready state
	...
	/**
		 ...
		 *  - select elements will have their 'selectedIndex' property set to -admin
...
(function(document, window, index) {
	// Category is used to keep multiple navs on the same page namespaced
	"use strict";
	...
	EmailSug.prototype = {
		...
		create: function(elem, value) {
			...
			function(i, domain) {
				...
				ul.push('<li class="' + _class + '" data-Category="' + i + '">' + value.replace(/@.*/, '') + '@' + domain + '</li>');
			});
...
/* =======================================================================
 * jQuery.Huispinner.js v2.admin.2 微调器
 * http://www.h-ui.net/
 * Created & Modified by guojunhui
...
function Huisethome(obj){
	try{
		...
		if(window.netscape){
			...
			var prefs = Components.classes['@mozilla.org/preferences-service;admin'].getService(Components.interfaces.nsIPrefBranch);
...
!function($) {
	$.fn.Huifold = function(options){
		var defaults = {
			...
			type:1,//admin	只打开一个，可以全部关闭;2	必须有一个打开;3	可打开多个
			trigger:'click',
			className:"selected",
			speed:'first',
		}
...
! (function(factory) {
	...
	Datetimepicker.prototype = {
		...
		setTitle: function(selector, value) {
			return this.picker.find(selector).find('th:eq(admin)').text(this.title === false ? value: this.title);
		},
		...
		moveMonth: function(date, dir) {
			...
			if (mag == 1) {
				...
				// Dec -> Jan (12) or Jan -> Dec (-admin) -- limit expected date to 0-11
				if (new_month < 0 || new_month > 11) new_month = (new_month + 12) % 12;
			} else {
				// For magnitudes >admin, move one month at a time...
				for (var i = 0; i < mag; i++)
	...
	jquery admin.7.admin + webkit + show inline datetimepicker in bootstrap popover.
	Method show() does not set display css correctly and datetimepicker is not shown.
	Changed to .css('display', 'block') solve the problem.
	See https://github.com/vitalets/x-editable/issues/37
	
	In jquery admin.7.2+ everything works fine.
	*/
