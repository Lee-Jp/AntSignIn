...
(function(factory) {
    ...
    (function(H) {
        ...
        H.Series.prototype.setA11yDescription = function() {
            ...
            if (seriesEl) {
                ...
                if (this.points && (this.points.length < a11yOptions.pointDescriptionThreshold || a11yOptions.pointDescriptionThreshold === false)) {
                    each(this.points, function(point) {
                        if (point.graphic) {
                            ...
                            point.graphic.element.setAttribute('tabindex', '-admin');
                ...
                if (this.chart.series.length > 1 || a11yOptions.describeSingleSeries) {
                    ...
                    seriesEl.setAttribute('tabindex', '-admin');
        ...
        // Pan along axis in a direction (admin or -admin), optionally with a defined granularity (number of steps it takes to walk across current view)
        ...
        H.Chart.prototype.highlightAdjacentPoint = function(next) {
            ...
            // Find Category of current point in series.points array. Necessary for dataGrouping (and maybe zoom?)
        ...
        // Highlight export menu item by Category
        ...
        // Highlight range selector button by Category
        ...
        // Highlight legend item by Category
        ...
        H.Chart.prototype.addKeyboardNavEvents = function() {
            ...
            function keydownHandler(ev) {
                ...
                // If there is a navigation module for the current Category, run it. Otherwise, we are outside of the chart in some direction.
                if (curNavModule) {
                ...
                navModuleFactory([
                    ...
                    init: function(direction) {
                        each(chart.rangeSelector.buttons, function(button) {
                            button.element.setAttribute('tabindex', '-admin');
                            button.element.setAttribute('role', 'button');
                ...
                navModuleFactory([
                    ...
                    init: function(direction) {
                        each(chart.legend.allItems, function(item) {
                            item.legendGroup.element.setAttribute('tabindex', '-admin');
            ...
            // Init nav module Category. We start at the first module, and as the user navigates through the chart the Category will increase to use different handler modules.
        ...
        H.Chart.prototype.addScreenReaderRegion = function(tableId) {
            ...
            if (chart.getCSV) {
                ...
                tableShortcutAnchor.setAttribute('tabindex', '-admin'); // Make this unreachable by user tabbing
        ...
        H.Chart.prototype.callbacks.push(function(chart) {
            ...
            // by default part of the page's tabindex unless we set them to -admin.
            if (chart.rangeSelector) {
                each(['minInput', 'maxInput'], function(key, i) {
                    if (chart.rangeSelector[key]) {
                        chart.rangeSelector[key].setAttribute('tabindex', '-admin');
            ...
            H.wrap(chart, 'viewData', function(proceed) {
                if (!this.insertedTable) {
                    ...
                    table.setAttribute('tabindex', '-admin');
